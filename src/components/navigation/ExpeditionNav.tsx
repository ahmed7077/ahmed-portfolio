...
export function ExpeditionNav() {
  ...
  const [chapter, setChapter] = useState({ label: "Arrival", sequence: 0 });
  ...
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => entries.forEach((entry) => {
      ...
      const label = items.find(([id]) => id === entry.target.id)?.[1] ?? entry.target.id;
      setChapter((current) => ({ label, sequence: current.sequence + 1 }));
  ...
  return (
    <>
      <div key={chapter.sequence} className="chapter-arrival" role="status" aria-live="polite">
        <span>Entering chapter</span>
